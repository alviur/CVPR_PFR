...
class Logger(ExperimentLogger):
    def __init__(self, log_path, exp_name, begin_time=None):
        ...
        sys.stdout = FileOutputDuplicator(sys.stdout, os.path.join(self.exp_path, 'stdout.txt'), 'w')
        sys.stderr = FileOutputDuplicator(sys.stderr, os.path.join(self.exp_path, 'stderr.txt'), 'w')
        # Raw log file
        self.raw_log_file = open(os.path.join(self.exp_path, "raw_log.txt"), 'w')
    ...
    def log_args(self, args):
        with open(os.path.join(self.exp_path, 'args.txt'), 'w') as f:
            json.dump(args.__dict__, f, separators=(',\n', ' : '), sort_keys=True)

    def log_result(self, array, name, step):
        if array.ndim <= 1:
            array = array[None]
        np.savetxt(os.path.join(self.exp_path, 'results', '{}.txt'.format(name)), array, '%.6f', delimiter='\t')

    def log_figure(self, name, iter, figure, curtime=None):
        curtime = datetime.now()
        figure.savefig(os.path.join(self.exp_path, 'figures', '{}_{}-{}.png'.format(name, iter, curtime.strftime("%Y-%m-%d-%H-%M-%S"))), 
                       bbox_inches='tight')
        figure.savefig(os.path.join(self.exp_path, 'figures', '{}_{}-{}.pdf'.format(name, iter, curtime.strftime("%Y-%m-%d-%H-%M-%S"))),
                       bbox_inches='tight')
