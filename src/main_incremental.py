...
def main(argv=None):
    ...
    for t, (_, ncla) in enumerate(taskcla):
        ...
        SS_APPROACHES = [args.approach]# evalSimsiam, simsiam, simsiamTiny,simsiamTinyPara
        if args.approach in SS_APPROACHES and (args.eval_omni_head or args.eval_fresh_head) and t>=0:
            if args.eval_fresh_head:
                ...
                for _t, (_, ncla) in enumerate(taskcla):
                    if _t < 10:
                        ...
                        # test_loss, fresh_head_acc_taw[t, _t], fresh_head_acc_tag[t, _t], matrices = \
                        #    appr.eval(_t, tst_loader[_t], heads_to_evaluate=fresh_heads)
                        ...
                        #ts = int(time.time())
                        #torch.save(matrices, f'{output_path}/eval-task_{t}-eval_task_{_t}-time_{ts}.pb')
            ...
            models_to_save = {
                ...
                #'simsiam': appr.modelLB.state_dict()

            }
            # if appr.modelFB.p2 is not None:
            #     models_to_save['p2'] = appr.modelFB.p2.state_dict()
            torch.save(models_to_save, f'{output_path}/model-task_{t}-time_{ts}.ckpt')
