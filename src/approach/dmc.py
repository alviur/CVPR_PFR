...
class Appr(Learning_Appr):
    ...
    described in http://openaccess.thecvf.com/content_WACV_2020/papers/Zhang_Class-incremental_Learning_via_Deep_Model_Consolidation_WACV_2020_paper.pdf
    Original code at https://github.com/juntingzh/incremental-learning-baselines"""
    ...
    def __init__(self, model, device, nepochs=160, lr=0.1, lr_min=1e-4, lr_factor=10, lr_patience=8, clipgrad=10000,
                 ...
                 aux_dataset='imagenet_32', aux_batch_size=128):
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--aux_dataset', default='imagenet_32_reduced', type=str, required=False,
                            help='(default=%(default)s)')
        parser.add_argument('--aux_batch_size', default=128, type=int, required=False,
                            help='(default=%(default)s)')
        return parser.parse_known_args(args)

    # Returns the optimizer
    def _get_optimizer(self):
        if self.exemplars_dataset is not None and len(self.exemplars_dataset) == 0 and len(self.model.heads) > 1:
    ...
    # Runs before training all the epochs of the task of new model's training
    ...
    # Runs after training all the epochs of the task of the student's training
...
class NewTaskTrainer(Learning_Appr):
    def __init__(self, model, device, nepochs=160, lr=0.1, lr_min=1e-4, lr_factor=10, lr_patience=8, clipgrad=10000,
...
class StudentTrainer(Learning_Appr):
    ...
    def criterion(self, t, outputs, targets_old, targets_new):
