...
class Appr(Learning_Appr):
    ...
    def extra_parser(args):
        ...
        parser.add_argument("--kd_method", default="ft", type=str, choices=['ft', 'L2', 'L2rel', 'L2relCos', 'p2', 'EWC', 'p2_f'])
    ...
    def get_data_loaders(self, trn_loader, val_loader, t):  # -> Replace _prepare_transformations
        ...
        #normalization = transforms.Normalize((0.5071, 0.4866, 0.4409), (0.2009, 0.1984, 0.2023)) # CIFAR100
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        if t == 0:
            ...
            torch.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:10002',
                                                 world_size=1, rank=0)
            ...
            if self.loadTask1:
                path = '/home/agomezvi/simSiamLB2/tinyP2_f/'#103
            ...
            #self.modelFB.cuda()
            #self.modelFB = torch.nn.parallel.DistributedDataParallel(self.modelFB)
            ...
            self.init_lr = 6e-2 * 0.4


            self.optimizer, self.scheduler = self.modelFB.configure_optimizers()

            #path = '/home/agomezvi/simSiamLB2/tinyP2_f/'
            #path = '/home/agomezvi/simSiamLB2/imagenetModels/'
            path = ''
            modelDict = torch.load(path + 'task_'+str(t-1)+'.pth.tar', map_location='cuda:0')
            ...
            #self.modelFB.cuda()
            #self.modelFB = torch.nn.parallel.DistributedDataParallel(self.modelFB, find_unused_parameters=True)
        ...
        # Save old models
        # self.modelFB.oldBackbone = deepcopy(self.modelFB.module.backbone)
        # self.modelFB.oldProjector = deepcopy(self.modelFB.module.projector)
        #
        # for param in self.modelFB.oldBackbone.parameters():
        #     param.requires_grad = False
        # for param in self.modelFB.oldProjector.parameters():
    ...
    def train_loop(self, t, trn_loader, val_loader, epoch):
        ...
        for i, ((x0, x1), _, _) in enumerate(self.dataloader_train_ssl):
            ...
            if t>0 and self.kd_method == "p2":
                ...
                p1, z2, p2, z1, f1Old, f2Old, p2_1, p2_2 = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                loss = -(self.criterion(p1, z2.detach()).mean() + self.criterion(p2, z1.detach()).mean()) * 0.5
                wandb.log({"Contrast loss": loss.item()})
                ...
                p1, z2, p2, z1 = self.modelFB(x1=x0.to(self.device), x2=x1.to(self.device))
                loss = -(self.criterion(p1, z2.detach()).mean() + self.criterion(p2, z1.detach()).mean()) * 0.5
                wandb.log({"Contrast loss": loss.item()})
...
class SimSiam(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        if self.t > 0 and self.kd == 'p2':
            ...
            return p1, z2.detach(),p2, z1.detach(),p2_1, z1Old,p2_2, z2Old
            ...
            return p1, z2.detach(),p2, z1.detach(),f1Old,f2Old,f1,f2
        #
            ...
            return p1, z2.detach(), p2, z1.detach(), self.fisher,self.older_params,self.backbone.cuda()
            ...
            return p1, z2.detach(), p2, z1.detach(), f1Old, f2Old, p2_1, p2_2
