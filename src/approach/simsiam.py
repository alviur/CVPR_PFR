...
class Appr(Learning_Appr):
    ...
    def train(self, t, trn_loader, val_loader):
        ...
        if t == 0:
            ...
            if self.loadTask1:
                path = '/home/agomezvi/simSiamLB2/modelsProj512/FT/'  # 512Proj
                # use https://drive.google.com/drive/folders/11_sc4N7MiR_z58ByHbQ7PwDeLq_Xf6bZ?usp=sharing for task 0
                #path = '/home/agomezvi/simSiamLB2/modelsProj2048/FT/'  # 2048Proj
                #path = '/home/agomezvi/simSiamLB4/result_simsiam/cifar100_noTrans_simsiam_simsiam_p2-10152021_172532_697059/' # ligtlys
                #path = '/home/agomezvi/simSiamLB2/result_simsiam/cifar100_noTrans_simsiam_simsiam_p2-10072021_150917_368433/'
...
class SimSiam(nn.Module):
    ...
    def __init__(self, base_encoder, dim, pred_dim, num_classes, tbwriter,
        ...
        if self.kd == 'p2' or self.kd == 'EWC_p2':
            ...
            #self.p2 = nn.Linear(512, 512, bias=False)
    ...
    def forward(self, x1, x2):
        ...
        if self.t > 0 and self.kd == 'p2_f':
        ...
        elif self.t > 0 and self.kd == 'EWC_p2':

            self.sampling_type = 'contrastive_lwf'

            lossKDEWC = self.criterionEWC(self.t)
            loss += lossKDEWC

            if self.wandblog:
                wandb.log({"KD EWC loss": lossKDEWC.item()})
            ...
            # if self.wandblog:
            wandb.log({"KD P2 loss": lossKD.item()})

            loss += lossKD

        elif self.t > 0 and self.kd == 'lwf_v3':
            ...
            lossKD = self.lambdap2 * (-(self.criterion(p1, z1Old.detach()).mean()
                            + self.criterion(p2, z2Old.detach()).mean()) * 0.5)
            loss += lossKD

        lossC = -(self.criterion(p1, z2.detach()).mean() + self.criterion(p2, z1.detach()).mean()) * 0.5

        loss += lossC

        if self.wandblog:
            wandb.log({"Total loss": lossC.item()})
